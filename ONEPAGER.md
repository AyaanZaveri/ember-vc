...
The AI wrote a helper that automatically wrapped multi-word search terms in quotes, on the assumption that quoting a phrase like "local news" would make the search more precise. It looked correct and passed every type check. But a quoted multi-word phrase inside a parenthesized OR-group, combined with a multi-word topic, silently returns zero results from Firecrawl's search every time. One of the regional-press probe terms was exactly this two-word phrase, so that entire category came back empty on every single run. It never looked like a bug: zero results for regional press just looked like the topic had no local coverage.

I caught it by noticing a suspicious `0` on a probe line in the live search trace, then testing the exact query directly against the Firecrawl API by hand. Removing the quotes, with everything else identical, took that query from 0 results to 6.

The lesson: AI-written query logic can look completely reasonable and still be silently wrong against the real behavior of an external API. Type-checking and code review wouldn't have caught this. The only way was to test the actual query against the live endpoint and look at what came back.
