...
While building the category-probe search templates, the AI wrote a helper that auto-quotes any multi-word search term, on the reasonable assumption that quoting a phrase like "local news" would make the search more precise. It looked correct and passed every type check. But against Firecrawl's actual search endpoint, a quoted multi-word phrase inside a parenthesized OR-group, combined with a multi-word topic, unpredictably returns zero results. Because one of my fixed template terms for regional press was the two-word phrase "local news," this silently killed that entire category on every single run all session. It never looked like a bug: zero results for regional press just looked like the topic had no local coverage.

I caught it by noticing a `· 0` on a probe line in the live search trace and testing the exact query directly against the Firecrawl API with a small script. Removing the quotes around the same phrase, with everything else identical, took that query from 0 results to 6. The same kind of isolated test caught a second issue: anchoring the forum probe on the full topic phrase including a B2B framing word like "manufacturers" steered results toward supplier directories instead of actual discussion threads. Stripping that one word, same operators otherwise, took genuine forums in the top 8 results from 2 to 6.

![Two query bugs, isolated and fixed](completeness-eval/figures/05_query_bugfixes.png)

Both of these are same-second A/B tests against the live API, one token changed at a time, not a noisier end-to-end comparison. The lesson: AI-written query logic can look completely reasonable and still be silently wrong against the real behavior of an external API. Type-checking and code review would never have caught this. The only way to catch it was to test the actual queries against the live search endpoint and look at what came back.
